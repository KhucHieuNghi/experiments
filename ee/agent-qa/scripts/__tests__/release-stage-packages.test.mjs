...
test('stages CLI package with packaged skills and without source-only lifecycle hooks', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'etus-agent-stage-packages-'))
  try {
    ...
    assert.equal(pkg.name, 'etus-agent')
    assert.equal(pkg.version, '0.1.1')
    assert.equal(pkg.scripts.prepack, undefined)
    assert.equal(pkg.scripts['copy:skills'], undefined)

    for (const skillName of ['etus-agent-authoring', 'etus-agent-result-triage', 'etus-agent-debug-fix']) {
