...
test('builds a non-mutating patch release dry-run plan', () => {
  ...
  assert.equal(plan.npm.publishCommands.at(-1).package, 'etus-agent')
  ...
  assert.equal(plan.subscriptionAuth.package, '@etus/agent-subscription-auth')
  assert.equal(plan.subscriptionAuth.status, 'dispatched_from_main_release_workflow')
  assert.ok(plan.subscriptionAuth.note.includes('etus-agent/.github/workflows/release.yml'))
  assert.ok(plan.subscriptionAuth.note.includes(targetVersion))
  assert.equal(plan.githubRelease.repository, 'etus/etus-agent')
  assert.equal(plan.githubRelease.tag, `v${targetVersion}`)
  assert.equal(plan.githubRelease.title, `etus-agent v${targetVersion}`)
...
test('CLI renders human-readable and JSON dry-run output without leaking secrets', async () => {
  ...
  assert.match(textOutput, /etus-agent release dry-run/)
  ...
  assert.match(textOutput, /etus-agent\/\.github\/workflows\/release\.yml/)
