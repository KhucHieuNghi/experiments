...
test('treats successful npm view output as an already-published version collision', async () => {
  await assert.rejects(
    checkNpmVersionsAbsent(['etus-agent'], '0.1.1', {
      execFileSync: () => '"0.1.1"',
    }),
    /npm version already published: etus-agent@0\.1\.1/,
  )
})

test('treats npm E404 as version absence', async () => {
  const calls = []
  await checkNpmVersionsAbsent(['etus-agent'], '0.1.1', {
  ...
  assert.deepEqual(calls, [['npm', ['view', 'etus-agent@0.1.1', 'version', '--json']]])
})
...
test('fails closed for ambiguous npm registry errors', async () => {
  await assert.rejects(
    ...
    /could not verify npm version absence for etus-agent@0\.1\.1/,
  )
})
