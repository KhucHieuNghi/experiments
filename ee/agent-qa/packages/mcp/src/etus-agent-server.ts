...
const DISCOVERY = {
  ...
  tools: [
    'etus_agent_discover',
    'etus_agent_get_config',
    'etus_agent_schema_reference',
    'etus_agent_validate_definition',
    'etus_agent_generate_id',
    'etus_agent_validate_id',
    'etus_agent_list_tests',
    'etus_agent_read_test',
    'etus_agent_validate_test',
    'etus_agent_create_test',
    'etus_agent_update_test',
    'etus_agent_delete_test',
    'etus_agent_list_suites',
    'etus_agent_read_suite',
    'etus_agent_validate_suite',
    'etus_agent_create_suite',
    'etus_agent_update_suite',
    'etus_agent_delete_suite',
    'etus_agent_list_hooks',
    'etus_agent_read_hook',
    'etus_agent_create_hook',
    'etus_agent_update_hook',
    'etus_agent_delete_hook',
    'etus_agent_run_hook',
    'etus_agent_enqueue_test_run',
    'etus_agent_enqueue_suite_run',
    'etus_agent_get_run',
    'etus_agent_get_run_steps',
    'etus_agent_get_run_logs',
    'etus_agent_get_run_execution_logs',
    'etus_agent_get_run_artifact',
    'etus_agent_cancel_run',
    'etus_agent_classify_failure',
  ],
  resources: SCHEMA_REFERENCE_NAMES.map(name => `etus-agent://schema/${name}`),
  prompts: ['etus_agent_authoring_context'],
  phases: {
...
export function categorizeMcpTool(name: string): McpToolCategory {
  if (name === 'etus_agent_discover') return 'discovery'
...
async function readConfig(configPath?: string): Promise<{
  ...
  const path = resolve(configPath ?? 'etus-agent.config.yaml')
...
export function createAgentQaMcpServer(options: AgentQaMcpServerOptions = {}): McpServer {
  ...
  >(
    ...
    try {
      ...
      const enqueueRunId = !isError && (name === 'etus_agent_enqueue_test_run' || name === 'etus_agent_enqueue_suite_run')
  ...
  registerAnalyticsTool(
    'etus_agent_get_config',
    {
      ...
      inputSchema: {
        configPath: z.string().optional().describe('Config path; defaults to the MCP server config path or etus-agent.config.yaml'),
  ...
  registerAnalyticsTool(
    ...
    async ({ schema }) => jsonContent(ETUS_AGENT_SCHEMA_REFERENCES[schema as SchemaReferenceName]),
  )
  ...
  for (const schema of SCHEMA_REFERENCE_NAMES) {
    server.registerResource(
      `etus_agent.schema.${schema}`,
      `etus-agent://schema/${schema}`,
      {
      ...
      async uri => ({
        contents: [{
          uri: uri.href,
          mimeType: 'application/json',
          text: JSON.stringify(ETUS_AGENT_SCHEMA_REFERENCES[schema], null, 2),
        }],
      }),
    )
  }
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            'Call etus_agent_get_config to inspect targets/devices/providers.',
            'Call etus_agent_generate_id for test, suite, hook, run, and observation IDs.',
            'Call etus_agent_validate_definition before saving config, test, suite, or hooks YAML.',
