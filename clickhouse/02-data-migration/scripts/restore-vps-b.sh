...
restore_schemas() {
    ...
    # Restore tables (two passes: data tables first, then Views/MVs)
    local deferred_tables=()
    
    # Pass 1: Create data tables, dictionaries (non-View, non-MV)
    for db_dir in "$schemas_dir"/*/; do
        ...
        for table_file in "$db_dir"/*.sql; do
            ...
            # Detect if this is a View or Materialized View — defer to pass 2
            if [[ "$create_stmt" =~ ^CREATE[[:space:]]+(MATERIALIZED[[:space:]]+)?VIEW ]] || [[ "$create_stmt" =~ ^CREATE[[:space:]]+DICTIONARY ]]; then
                deferred_tables+=("$table_file")
                log "  Deferring: ${db_name}.${table_name} (View/MV/Dictionary — created after base tables)"
                continue
            fi
    ...
    # Pass 2: Create deferred objects (Views, Materialized Views, Dictionaries)
    if [[ ${#deferred_tables[@]} -gt 0 ]]; then
        log "Creating deferred objects (Views, MVs, Dictionaries)..."
        for table_file in "${deferred_tables[@]}"; do
            local db_name=$(basename "$(dirname "$table_file")")
            local table_name=$(basename "$table_file" .sql)
            local create_stmt=$(cat "$table_file")
            
            if [[ "$DRY_RUN" == "true" ]]; then
                echo "  [DRY-RUN] Would create: ${db_name}.${table_name}"
            else
                log "  Creating: ${db_name}.${table_name}"
                eval "$client_cmd -q \"$create_stmt\"" 2>&1 || {
                    warning "Failed to create: ${db_name}.${table_name}"
                    continue
                }
            fi
        done
...
restore_users() {
    ...
    # Prefer SQL-based restore (generated by updated backup_users)
    if [[ -f "${users_dir}/create_users.sql" ]] && [[ -s "${users_dir}/create_users.sql" ]]; then
        log "Restoring users from SQL scripts..."
        while IFS= read -r line; do
            [[ -z "$line" ]] && continue
            [[ "$line" == ";" ]] && continue
            log "  Executing: ${line:0:80}..."
            eval "$client_cmd -q \"$line\"" 2>&1 || warning "  Failed: $line"
        done < "${users_dir}/create_users.sql"
        
        # Restore grants
        if [[ -f "${users_dir}/grant_statements.sql" ]] && [[ -s "${users_dir}/grant_statements.sql" ]]; then
            log "Restoring grants from SQL scripts..."
            while IFS= read -r line; do
                [[ -z "$line" ]] && continue
                [[ "$line" == ";" ]] && continue
                eval "$client_cmd -q \"$line\"" 2>&1 || warning "  Failed: $line"
            done < "${users_dir}/grant_statements.sql"
        fi
        
        success "Users and grants restored from SQL scripts"
    elif [[ -f "${users_dir}/user_list.txt" ]]; then
        # Fallback: create users with temporary password (old backup format)
        log "No SQL scripts found — falling back to user_list.txt..."
        while IFS= read -r user; do
            [[ -z "$user" ]] && continue
            [[ "$user" == "default" ]] && continue
            
            log "  Creating user: $user"
            eval "$client_cmd -q \"CREATE USER IF NOT EXISTS \\\`${user}\\\` IDENTIFIED WITH sha256_password BY 'CHANGE_ME_IMMEDIATELY'\"" 2>&1 || {
                warning "Failed to create user: $user"
            }
        done < "${users_dir}/user_list.txt"
        
        warning "Users created with temporary password 'CHANGE_ME_IMMEDIATELY'"
        warning "Please update passwords and review grants manually"
    fi
    
    # Reference files for manual review
    log "  Reference files available:"
    [[ -f "${users_dir}/users.tsv" ]] && log "    - users.tsv: User definitions"
    [[ -f "${users_dir}/grants.tsv" ]] && log "    - grants.tsv: Grant data"
    [[ -f "${users_dir}/roles.tsv" ]] && log "    - roles.tsv: Role definitions"
    [[ -f "${users_dir}/quotas.tsv" ]] && log "    - quotas.tsv: Quota definitions"
    [[ -f "${users_dir}/settings_profiles.tsv" ]] && log "    - settings_profiles.tsv: Settings profiles"
